...
class MotionData(object):
  ...
  def calc_frame(self, time):
    ...
    frame0 = self.get_frame(f0) # NO.f0 row
    ...
    blend_root_rot = self.get_frame_root_rot(blend_frame) # how many rotation from 0 frame to f0/f1 frame

    cycle_count = self.calc_cycle_count(time)
    cycle_offset_pos = self._calc_cycle_offset_pos(cycle_count)
    cycle_offset_rot = self._calc_cycle_offset_rot(cycle_count) # how many rotation in so many cycles
  ...
  def calc_blend_idx(self, time):
    ...
    """
    dur = self.get_duration() # duration (seconds) of the entire motion
    num_frames = self.get_num_frames() # how many rows in total
    ...
    if not self.enable_loop() and time <= 0:
      ...
      phase = self.calc_phase(time) # in what phase in this montion(start 0, end 1)
