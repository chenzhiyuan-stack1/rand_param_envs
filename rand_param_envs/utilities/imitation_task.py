...
from rand_param_envs.utilities import imitation_terminal_conditions
from rand_param_envs.utilities import pose3d
from rand_param_envs.utilities import motion_data
...
class ImitationTask(object):
  ...
  def reset(self, env):
    ...
    self._curr_episode_warmup = False # state from the reference at the start of each episode
  ...
  def build_target_obs(self):
    ...
    sim_base_rot = self._env.model.data.qpos.ravel().copy()[3:7]
  ...
  def _calc_reward_pose(self):
    ...
    sim_model = self._env.model
    ref_model = self._env.ref_model
    ...
    for j in range(num_joints):
      ...
      j_state_ref = ref_model.data.qpos.ravel()[j + 7]
      j_state_sim = sim_model.data.qpos.ravel()[j + 7]
      j_pose_ref = np.array(j_state_ref)
      j_pose_sim = np.array(j_state_sim)
  ...
  def _calc_reward_velocity(self):
    ...
    for j in range(num_joints):
      # j_state_ref = pyb.getJointStateMultiDof(ref_model, j)
      # j_state_sim = pyb.getJointStateMultiDof(sim_model, j)
      j_state_ref = ref_model.data.qvel.ravel()[j + 6]
      j_state_sim = sim_model.data.qvel.ravel()[j + 6]
      # j_vel_ref = np.array(j_state_ref[1])
      # j_vel_sim = np.array(j_state_sim[1])
      j_vel_ref = np.array(j_state_ref)
      j_vel_sim = np.array(j_state_sim)
      ...
      if (j_size_ref > 0):
        ...
        j_vel_err = j_vel_diff * (j_vel_diff)
        vel_err += j_vel_err
  ...
  def _calc_reward_root_velocity(self):
    ...
    root_vel_ref = ref_model.data.qvel.ravel()[0:3]
    root_ang_vel_ref = ref_model.data.qvel.ravel()[3:6]
    root_vel_sim = sim_model.data.qvel.ravel()[0:3]
    root_ang_vel_sim = sim_model.data.qvel.ravel()[3:6]
  ...
  def _load_ref_motions(self, filenames):
    ...
    for filename in filenames:
      curr_motion = motion_data.MotionData(filename) # is MotionData type
      curr_duration = curr_motion.get_duration() # the total time duration of the data in dog_pace.txt
  ...
  def _build_ref_model(self):
    ...
    """
    # ref_col = [1, 1, 1, self._draw_ref_model_alpha] # use this color to blur the reference model
    ...
    #     ref_model, -1, collisionFilterGroup=0, collisionFilterMask=0) # ignore collision of base link
    ...
    # pyb.changeVisualShape(ref_model, -1, rgbaColor=ref_col) # blur the reference model
    ...
    # for j in range(num_joints):
    #   pyb.setCollisionFilterGroupMask(
    #       ref_model, j, collisionFilterGroup=0, collisionFilterMask=0) # ignore collision of all link
    ...
    #   pyb.changeVisualShape(ref_model, j, rgbaColor=ref_col) # blur the reference model
    ref_model = self._env.ref_model
  ...
  def _reset_ref_motion(self):
    ...
    motion = self.get_active_motion() # naming a row in dog_pace.txt
    time = self._get_motion_time()

    ref_pose = self._calc_ref_pose(time) # determine the pose of the model, apply_origin_offset=True
    ...
    self._origin_offset_rot = transformations.quaternion_about_axis(
        delta_heading, [0, 0, 1]) # critical, rotation delta_heading(-pi/2) around z axis
    self._ref_pose = self._calc_ref_pose(time) # why we get the wrong rotation value ?
  ...
  def _update_ref_motion(self):
    ...
    # 动作转一圈回来了，此处只同步position
    # 注意到这里motion是不会over的，因此每过一轮，都要sync一下
    # 而sync只是修改self._origin_offset_pos，在_calc_ref_pose(apply_origin_offset=True)时起作用
  ...
  def _update_ref_model(self):
    ...
    # print("set_state")
    # print(self._ref_pose[0],self._ref_pose[1],self._ref_pose[2])
    return
  ...
  def _sync_sim_model(self, perturb_state):
    ...
    self._set_state(self._env.model, pose, vel)
    return
  ...
  def _set_state(self, phys_model, pose, vel):
    ...
    # 这里使用的qpos和qvel都要是.ravel()过的
    assert qpos.shape == (phys_model.nq,) and qvel.shape == (phys_model.nv,)
    phys_model.data.qpos = qpos
    phys_model.data.qvel = qvel
    phys_model._compute_subtree()  # pylint: disable=W0212
    phys_model.forward() # 不计时间的mj_step()
    return
  ...
  def _get_sim_base_position(self):
    ...
    # pos = np.array(pos)
    pos = self._env.model.data.qpos.ravel()[0:3]
    return pos
  ...
  def _get_sim_base_rotation(self):
    ...
    rotation = self._env.model.data.qpos.ravel()[3:7]
    return rotation
  ...
  def _get_ref_base_position(self):
    ...
    # pos = np.array(pos)
    pos = self._env.ref_model.data.qpos.ravel()[0:3]
    return pos
  ...
  def _get_ref_base_rotation(self):
    ...
    # rotation = np.array(rotation)
    rotation = self._env.ref_model.data.qpos.ravel()[3:7]
    return rotation
  ...
  def _sync_ref_origin(self, sync_root_position, sync_root_rotation):
    ...
    time = self._get_motion_time()
    motion = self.get_active_motion()
    ref_pose = self._calc_ref_pose(time, apply_origin_offset=False) # 只读取txt里的数据，不做offset
  ...
  def _build_sim_pose(self, phys_model):
    ...
    root_pos = phys_model.data.qpos.ravel()[0:3]
    root_pos = phys_model.data.qpos.ravel()[3:7]
    ...
    for j in range(num_joints):
      ...
      j_state_sim = phys_model.data.qpos.ravel()[j + 7]
